...
from recpack.pipelines.hyperparameter_optimisation import (
    OptimisationInfo,
    GridSearchInfo,
)
...
from minipack.pipelines.entries import (
    AlgorithmEntry,
    MetricEntry,
    OptimisationMetricEntry,
)
from minipack.pipelines.training.pipeline import Pipeline
...
class PipelineBuilder(object):
    ...
    def __init__(
        self, folder_name: Optional[str] = None, base_path: Optional[str] = None
    ):
    ...
    def add_metric(
        self, metric: Union[str, type], K: Optional[Union[List, int]] = None
    ):
    ...
    def add_algorithm(
        ...
        self.algorithm_entries.append(
            AlgorithmEntry(algorithm, optimisation_info or None, params or {})
        )
    ...
    def _check_readiness(self):
        ...
        if not hasattr(self, "optimisation_metric") and np.any(
            [algo.optimise for algo in self.algorithm_entries]
        ):
            raise RuntimeError(
        ...
        if (
            not hasattr(self, "validation_training_data")
            and self._requires_validation_data()
        ):
            raise RuntimeError(
        ...
        if hasattr(self, "validation_training_data"):
            if isinstance(self.validation_training_data, tuple):
                if any([d.shape != shape for d in self.validation_training_data]):
                    raise RuntimeError(
                        "Shape mismatch between validation training data and full training data"
                    )
            elif self.validation_training_data.shape != shape:
                raise RuntimeError(
                    "Shape mismatch between validation training data and full training data"
                )
