...
from minipack.pipelines.registries import (
    MINIMIZATION_REGISTRY,
    METRIC_REGISTRY,
    TARGET_ESTIMATOR_REGISTRY,
)
from minipack.pipelines.entries import MinimizerEntry, MetricEntry
from minipack.pipelines.minimization.pipeline import Pipeline
...
class PipelineBuilder(object):
    ...
    def add_minimizers(self, config: List[Dict[str, Any]]):
        for minimizer in config:
            self.add_minimizer(minimizer["name"], minimizer["params"])

    def add_target_estimator(self, estimator: Union[str, type], params: Optional[Dict[str, Any]] = None):
        ...
        if target_estimator_cls is None:
            raise ValueError(f"Target estimator {estimator} could not be resolved.")

        self.target_estimator = target_estimator_cls(**params)
    ...
    def interaction_matrices(self):
        return self._interaction_matrices

    @interaction_matrices.setter
    def interaction_matrices(self, interaction_matrices: List[csr_matrix]):
        if type(interaction_matrices) != list:
            raise TypeError(f"Argument should be list of CSR Matrix, not {type(interaction_matrices)}")
        self._interaction_matrices = interaction_matrices

    @property
    def target_matrices(self):
        return self._target_matrices

    @target_matrices.setter
    def target_matrices(self, target_matrices: List[csr_matrix]):
        if type(target_matrices) != list:
            raise TypeError(f"Argument should be list of CSR Matrix, not {type(target_matrices)}")
        self._target_matrices = target_matrices

    @property
    def test_matrices(self):
        return self._test_matrices

    @test_matrices.setter
    def test_matrices(self, test_matrices: List[csr_matrix]):
        if type(test_matrices) != list:
            raise TypeError(
                f"Argument should be list of CSR Matrix, not {type(test_matrices)}"
            )
        self._test_matrices = test_matrices
    ...
    def save_results(self):
        if not isinstance(self._save_results, bool):
            raise TypeError(
                f"Save results should be a boolean, not {type(self._save_results)}"
            )
    ...
    def results_directory(self):
        if not isinstance(self._results_directory, str):
            raise TypeError(
                f"Results directory should be a string, not {type(self._results_directory)}"
            )
    ...
    def _check_readiness(self):
        ...
        if not hasattr(self, "target_matrices") and not hasattr(self, "target_estimator"):
            raise RuntimeError(
        ...
        if hasattr(self, "target_matrices"):
            if len(self.interaction_matrices) != len(self.target_matrices):
                raise RuntimeError(
                    "Length mismatch between input data and ground truth data, can't construct pipeline"
                )
            for i in range(len(self.interaction_matrices)):
                if self.interaction_matrices[i].shape[0] != self.target_matrices[i].shape[0]:
                    raise RuntimeError(
    ...
    def build(self) -> Pipeline:
        ...
        return Pipeline(
            ...
            self.target_matrices if hasattr(self, "target_matrices") else None,
            self.test_matrices if hasattr(self, "test_matrices") else None,
            self.model,
            self.minimizer_entries,
            list(self.metric_entries.values()),
            self.target_estimator if hasattr(self, "target_estimator") else None,
