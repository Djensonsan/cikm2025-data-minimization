...
from minipack.scenarios.splitters import (
    UniformlyStratifiedStrongGeneralizationSplitter,
...
class UniformStratifiedStrongGeneralization(Scenario):
    ...
    training, validation, estimation and testing. This is a stratified version of the
    StrongGeneralization scenario, where the users are stratified into bins based on their
    number of interactions.

    This is the equalized version of the StratifiedStrongGeneralization, which ensures each bin has the same number of users.
    The number of users per bin is calculated as the total number of users divided by the number of bins.
    ...
    def __init__(
        ...
        self.strat_strong_gen_test = UniformlyStratifiedStrongGeneralizationSplitter(
        ...
        if self.estimation:
            ...
            self.strat_strong_gen_est = UniformlyStratifiedStrongGeneralizationSplitter(
                self.num_users_est, self.bin_width, self.bin_range, seed=self.seed
            )
        if self.validation:
            if self.num_users_val <= 0:
                raise ValueError(
    ...
    def _split(self, data: InteractionMatrix) -> None:
        """Splits your data so that a user can only be in one of
            training, validation, estimation or test set.
