...
from minipack.minimizers import GreedyForwardMinimizer
...
def test_minimize_with_defaults(small_random_interaction_matrix, small_random_target_matrix):
    ...
    minimizer = GreedyForwardMinimizer(metric=NDCG(K=2), model=model, remove_history=False)
...
def test_minimize_with_max_size(small_random_interaction_matrix, small_random_target_matrix):
    """Test minimization with max_size parameter."""
    model = ItemKNN()
    model.fit(small_random_interaction_matrix)

    # Note: setting eta to 100 means that performance thresholds will be impossible to meet (unless performance was 0)
    minimizer = GreedyForwardMinimizer(metric=NDCG(K=2), eta=100, model=model, max_size=2, remove_history=True)
    result = minimizer.minimize(small_random_interaction_matrix, small_random_target_matrix)

    assert_common_results(result, small_random_interaction_matrix, small_random_target_matrix)
    assert np.all(result.per_user_minimized_input_interactions <= 2)
    assert result.batch_constraint_satisfaction <= 1.0
...
def test_minimize_timeout(small_random_interaction_matrix, small_random_target_matrix):
    ...
    minimizer = GreedyForwardMinimizer(metric=NDCG(K=2), model=model, remove_history=False, timeout=1e-9)
    result = minimizer.minimize(small_random_interaction_matrix, small_random_target_matrix)
...
def test_minimize_with_different_eta(
    ...
    minimizer = GreedyForwardMinimizer(metric=NDCG(K=2), model=model, eta=eta, remove_history=True)
