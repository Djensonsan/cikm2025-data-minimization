...
from minipack.minimizers import EmbeddingSimilaritySelectionMinimizer
...
def test_minimize_with_defaults(small_random_interaction_matrix, small_random_target_matrix):
    ...
    minimizer = EmbeddingSimilaritySelectionMinimizer(metric=NDCG(K=2), model=model, remove_history=False)
...
def test_minimize_timeout(large_random_interaction_matrix, large_random_target_matrix):
    ...
    minimizer = EmbeddingSimilaritySelectionMinimizer(metric=NDCG(K=2), model=model, remove_history=False, timeout=1e-9)
...
def test_minimize_with_different_eta(
    ...
    minimizer = EmbeddingSimilaritySelectionMinimizer(
        metric=NDCG(K=2), model=model, eta=eta
    )
