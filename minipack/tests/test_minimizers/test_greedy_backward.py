...
from minipack.minimizers import GreedyBackwardMinimizer
...
def test_minimize_with_defaults(small_random_interaction_matrix, small_random_target_matrix):
    """Test minimization with default settings on a randomized nearest neighbour model."""
    model = ItemKNN()
    model.fit(small_random_interaction_matrix)

    minimizer = GreedyBackwardMinimizer(
        metric=NDCG(K=2), model=model, remove_history=False
    )
...
def test_minimize_timeout(small_random_interaction_matrix, small_random_target_matrix):
    ...
    minimizer = GreedyBackwardMinimizer(metric=NDCG(K=2), model=model, timeout=1e-6)
    result = minimizer.minimize(small_random_interaction_matrix, small_random_target_matrix)
...
@pytest.mark.parametrize(
    ...
    [
        (0.0, 8, 0),  # eta=0.0 should remove all interactions
        (1.0, 8, 3),  # eta=1.0 should remove a single interaction
        (0.9, 8, 3),  # eta=0.9 should remove a single interaction
    ],
)
def test_minimize_with_different_eta(
    eta,
    expected_sample_count,
    expected_minimized_interactions,
    simple_interaction_matrix,
    simple_target_matrix,
    simple_train_matrix,
):
    """
    ...
    minimizer = GreedyBackwardMinimizer(metric=NDCG(K=2), model=model, eta=eta, remove_history=True)
