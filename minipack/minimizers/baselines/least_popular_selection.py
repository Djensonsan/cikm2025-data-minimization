...
class LeastPopularSelectionMinimizer(BaseMinimizer):
    ...
    def _setup(self, interaction_matrix: csr_matrix) -> tuple:
        ...
        item_pop = np.array(interaction_matrix.sum(axis=0)).flatten()
        item_pop = 1 / item_pop

        # contains the unpopularity for each user history
        popularity_matrix = interaction_matrix.multiply(item_pop).tolil()

        # Create a LIL matrix sorted by value (ascending) for efficient retrieval of the least popular item.
        # Unlike the default LIL order (sorted by column index), this may introduce overhead or side effects in other sparse operations.
        ...
        index_map = np.array(range(interaction_matrix.shape[0]), dtype=int)

        # holds rows that don't meet the performance threshold
        lookup_original_user_idx = np.array(range(interaction_matrix.shape[0]), dtype=int)
        return minimized_matrix, popularity_matrix, index_map, lookup_original_user_idx
    ...
    def _minimize(
        ...
        while True:
            self.result_builder.start_timer(
                user_ids=users_below_threshold, init_value=0.0
            )
            ...
            self.result_builder.set_scores(
                user_ids=users_below_threshold, scores=scores
            )

            # Identify users still below the threshold
            users_below_threshold = np.where(
                scores < self.thresholds[users_below_threshold]
            )[0]
